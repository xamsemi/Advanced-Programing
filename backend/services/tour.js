...
serviceRouter.get('/api/tour/:id', function(request, response) {
    ...
    try {
        ...
        console.error('Service Book: Error loading record by id. Exception occured: ' + ex.message);
...
serviceRouter.post('/api/tour/', upload.single('file'), function(request, response) {
    ...
    if (errorMsgs.length > 0) {
        console.log('Service Book: Creation not possible, data missing');
    ...
    try {
        ...
        console.log('Service Book: Record created');
        response.status(200).json(obj);
    } catch (ex) {
            console.error('Service Book: Error creating record. Exception occurred: ' + ex.message);
            response.status(400).json({ 'fehler': true, 'nachricht': ex.message });
    }
});

serviceRouter.post('/api/book/:id', upload.single('file'), function(request, response) {
    console.log('Service Book: Client requested update of record, id=' + request.params.id);
    ...
    if (errorMsgs.length > 0) {
        console.log('Service Book: Update not possible, data missing');
        console.log(errorMsgs);
        response.status(400).json({ 'fehler': true, 'nachricht': 'Funktion nicht möglich. Fehlende Daten: ' + helper.concatArray(errorMsgs) });
        return;
    }
    ...
    try {
        ...
        console.log('Service Book: Record updated');
        response.status(200).json(obj);
    } catch (ex) {
            console.error('Service Book: Error updating record. Exception occurred: ' + ex.message);
            response.status(400).json({ 'fehler': true, 'nachricht': ex.message });
    }
});


serviceRouter.delete('/api/book/:id', function(request, response) {
    console.log('Service Book: Client requested deletion of record, id=' + request.params.id);

    const bookDao = new TourDao(request.app.locals.dbConnection);
    try {
        var obj = bookDao.loadById(request.params.id);
        bookDao.delete(request.params.id);
        console.log('Service Book: Deletion of record successfull, id=' + request.params.id);
        response.status(200).json({ 'gelöscht': true, 'eintrag': obj });
    } catch (ex) {
        console.error('Service Book: Error deleting record. Exception occured: ' + ex.message);
        response.status(400).json({ 'fehler': true, 'nachricht': ex.message });
    }
});
